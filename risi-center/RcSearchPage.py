...
class RcSearchPage(Gtk.ScrolledWindow):
    ...
    def load_page(self): # Function loads up thread
        thread = threading.Thread(target=self.app_thread, args=[self.apps])
        thread.daemon = True
        thread.start()
    ...
    def app_thread(self, apps): # The thread for loading apps to prevent freezing.
        for app in apps:
            ...
            time.sleep(0.01) # Sleep prevents GTK from trying to load all the apps at once
